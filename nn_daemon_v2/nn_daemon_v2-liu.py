...
#默认重启周期天数
DEFAULT_REBOOT_DAY = 0
#默认重启时间段
DEFAULT_REBOOT_TIME = "00:00-07:00"

#重启周期天数设为0或为负时，则不重启程序，只对其进行监控。若进程不存在，则启动它。
#INFINITE_DAY 表示一个很大的天数。重启周期天数为0或为负时，修改其值为INFINITE_DAY
INFINITE_DAY = 365
...
def get_all_task():
	...
	for section in sections:
		task_info[section] = []
		try:
			task_info[section].append(cf.get(section, "path"))
		except Exception,e:
		...
		try:
			task_info[section].append(cf.get(section, "reboot_day"))
		except Exception,e:
			task_info[section].append(DEFAULT_REBOOT_DAY)
			logging.error("%s"%e)
			logging.info("set default reboot day to %s"%section)
		...
		try:
			...
			if int(reboot_time_2[0]) < 0 or int(reboot_time_2[0]) > 23 or int(reboot_time_2[1]) < 0 or int(reboot_time_2[1]) > 59:
				logging.error("%s's reboot_time %s is not in range,set default reboot time: %s"%(section, reboot_time, DEFAULT_REBOOT_TIME))
				task_info[section].append(parser_time_1(str(DEFAULT_REBOOT_TIME)))
			else:
				task_info[section].append(reboot_time_1)
		except Exception,e:
			task_info[section].append(parser_time_1(str(DEFAULT_REBOOT_TIME)))
			logging.error("%s"%e)
			logging.info("%s's reboot_time is not set, set default reboot time: %s"%(section, DEFAULT_REBOOT_TIME))
...
def is_end_time_bigger_than_begin_time(time_list):
	...
	if begin_time > end_time:
		logging.warning("Please check the reboot_time,begin_time and end_time must be in a day.")
		return False
	elif begin_time == end_time:
		logging.warning("Reboot time can not be a time point.")
		return False
	else: 
		return True
...
if __name__ == "__main__":
	...
	for key in task_info:
		if int(task_info[key][1]) == DEFAULT_REBOOT_DAY:
			logging.info("task:%s not need to reboot"%task_info[key][0])
			continue
